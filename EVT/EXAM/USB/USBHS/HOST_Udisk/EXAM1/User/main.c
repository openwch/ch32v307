                ...
                if( ret == ERR_SUCCESS )
                {
                    ...
                    for ( i = 0; i != 10; i ++ )
                    {
                          ...
                          if ( ret == ERR_SUCCESS )
                          {
                              ...
                              /* 读文件 */
                              strcpy( (PCHAR)mCmdParam.Open.mPathName, "/NEWFILE.C" );  //设置将要操作的文件路径和文件名/C51/NEWFILE.C
                              ret = CHRV3FileOpen( );                                       //打开文件
                              if ( ret == ERR_MISS_DIR || ret == ERR_MISS_FILE )            //没有找到文件
                              {
                                  //创建文件演示
                                  printf( "Find No File And Create\r\n" );
                                  strcpy( (PCHAR)mCmdParam.Create.mPathName, "/NEWFILE.C" );          /* 新文件名,在根目录下,中文文件名 */
