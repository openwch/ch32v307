...
__attribute__ ((aligned(4))) UINT8   RxBuffer[ 64  ];  // IN, must even address
__attribute__ ((aligned(4))) UINT8   TxBuffer[ 64  ];  // OUT, must even address
...
void USBOTG_HostInit(FunctionalState stus)
{
    ...
    {
        OTG_RCC_Init();
        USBOTG_H_FS->BASE_CTRL = USBHD_UC_HOST_MODE;
        USBOTG_H_FS->HOST_CTRL = 0;
        USBOTG_H_FS->DEV_ADDR=0;
        USBOTG_H_FS->HOST_EP_MOD = USBHD_UH_EP_TX_EN | USBHD_UH_EP_RX_EN;

        USBOTG_H_FS->HOST_RX_DMA = pHOST_RX_RAM_Addr;
        USBOTG_H_FS->HOST_TX_DMA = pHOST_TX_RAM_Addr;
...
UINT8 USBHostTransact( UINT8 endp_pid, UINT8 tog, UINT32 timeout )
{
    ...
    do {
        ...
        if( USBOTG_H_FS->INT_FG & USBHD_UIF_DETECT ) {                           /* 当前设备被拔除 */
            ...
            if ( CHRV3DiskStatus == DISK_DISCONNECT ) return( ERR_USB_DISCON );
            if ( CHRV3DiskStatus == DISK_CONNECT )    return( ERR_USB_CONNECT );
        ...
        {
            if ( USBOTG_H_FS->INT_ST & USBHD_UIS_TOG_OK ){
                return( ERR_SUCCESS );                                          /* 数据包令牌匹配 */
            }
...
UINT8 HostCtrlTransfer( PUINT8 DataBuf, PUINT8 RetLen )
{
    ...
    if ( RemLen && pBuf )                                                                                    /* 判断是否有数据阶段 */
    {
        if ( pSetupReq -> bRequestType & USB_REQ_TYP_IN )                                                    /* 数据阶段IN事务 */
        {
            ...
            {
                Delay_Us( 200 );
                USBOTG_H_FS->HOST_RX_DMA = (UINT32)pBuf+*pLen;
    ...
    s = USBHostTransact(  (USBOTG_H_FS->HOST_TX_LEN) ? (USB_PID_IN << 4 | 0x00): (USB_PID_OUT << 4 | 0x00) ,
            USBHD_UH_T_TOG | USBHD_UH_R_TOG, 200000/20 );
    if ( s != ERR_SUCCESS )                      return( s );
...
UINT8 CtrlGetConfigDescr( PUINT8 DataBuf )
{
    ...
    if ( s != ERR_SUCCESS )                return( s );
//    Analysis_Descr(&ThisUsbDev, DataBuf, len);
