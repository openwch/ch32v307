...
#define UART_REV_LEN       64     //USB串口接收缓冲区最大字节数

extern u8  Receive_Uart_Buf[UART_REV_LEN]; //接收缓冲，最大USB_USART_REC_LEN个字节.末字节为换行符
extern u8  Uart_Input_Point;   //循环缓冲区写入指针，总线复位需要初始化为0
extern u8  Uart_Output_Point;  //循环缓冲区取出指针，总线复位需要初始化为0
extern u8  UartByteCount;     //当前缓冲区剩余待取字节数
extern u8  EP4_DatabufHD[64+64];

extern u8  USBByteCount;     //代表USB端点接收到的数据
extern u8  USBBufOutPoint;    //取数据指针

extern u8  UpPoint4_Busy;    //上传端点是否忙标志
...
extern volatile UINT16 USBHD_Endp3_Up_Flag;
extern volatile UINT8  USBHD_Endp3_Down_Flag;
extern volatile UINT8  USBHD_Endp3_Down_Len;
extern volatile BOOL   USBHD_Endp3_T_Tog;
extern volatile BOOL   USBHD_Endp3_R_Tog;
/******************************************************************************/
/* DMA地址和缓冲区相关宏定义 */
