...
static rt_err_t ch32_spi_init(struct ch32_spi *spi_drv, struct rt_spi_configuration *cfg)
{
    ...
    {
        spi_handle->Init.SPI_Direction = (SPI_Direction_1Line_Rx|SPI_Direction_1Line_Tx);
    }
    else
    {
...
//    SPI_I2S_ITConfig(spi_handle->Instance,SPI_I2S_IT_TXE|SPI_I2S_IT_RXNE,DISABLE);
...
uint8_t HAL_SPI_TransmitReceive(SPI_HandleTypeDef *handle,uint8_t *send_buf,uint8_t *recv_buf,uint16_t send_length,uint16_t timeout)
{
    uint16_t i=0;
    uint8_t ret=0;
   do
   {
        while (SPI_I2S_GetFlagStatus(handle->Instance, SPI_I2S_FLAG_TXE) == RESET)
        {
            i++;
            if(i>timeout){ret=1; break;}
        }
        SPI_I2S_SendData(handle->Instance, *send_buf++);    /* send byte */

        i=0;
        while (SPI_I2S_GetFlagStatus(handle->Instance, SPI_I2S_FLAG_RXNE) == RESET)
        {
            i++;
            if(i>timeout){ret=1; break ;}
        }
        *recv_buf++ =SPI_I2S_ReceiveData(handle->Instance);/* receive byte */
   }while(send_length--);

   return ret;
}
uint8_t HAL_SPI_Transmit(SPI_HandleTypeDef *handle, uint8_t *send_buf,uint16_t send_length, uint16_t timeout)
{
    uint16_t i=0;
    uint8_t ret=0;
    do
    {
        ...
        SPI_I2S_SendData(handle->Instance, *send_buf++);
    }while(send_length--);
    return ret;
}

uint8_t HAL_SPI_Receive(SPI_HandleTypeDef *handle, uint8_t *recv_buf, uint16_t send_length, uint16_t timeout)
{
    uint16_t i=0;
    uint8_t ret=0;
    do
...
static void ch32_get_dma_info(void)
{
